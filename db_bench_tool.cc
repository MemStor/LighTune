...
// 定义监听的文件路径
const std::string OPTION_FILE_PATH = "/tmp/rocksdb_dyn_params";

// 获取文件最后修改时间
long GetFileModTime(const std::string& path) {
    struct stat result;
...
// 动态修改线程的主体逻辑
void DynamicOptionListener(rocksdb::DB* db, std::atomic<bool>* running) {
    long last_mod_time = GetFileModTime(OPTION_FILE_PATH);
    if (db == nullptr) {
        fprintf(stderr, "Error: DynamicListener received a null DB pointer! Thread exiting.\n");
        return;
    }
    fprintf(stderr, ">> Dynamic Listener started. Watching file: %s\n", OPTION_FILE_PATH.c_str());

    while (*running) {
        std::this_thread::sleep_for(std::chrono::seconds(2)); // 每2秒检查一次

        long current_mod_time = GetFileModTime(OPTION_FILE_PATH);
        if (current_mod_time > last_mod_time) {
            last_mod_time = current_mod_time;
            
            // 读取文件内容
            std::ifstream file(OPTION_FILE_PATH);
            std::string line;
            std::unordered_map<std::string, std::string> options_map;
	    while (std::getline(file, line)) {
                // 1. 去除整行的首尾空白（避免空行包含空格导致误判）
                line = Trim(line);
    ...
    for (const auto& kv : options_map) {
        ...
        if (key.find("level") == 0 ||
            key.find("write_buffer") == 0 ||
            key.find("max_bytes_for_level") == 0) {

            cf_options[kv.first] = kv.second;

        } else {

            db_options[kv.first] = kv.second;
        }
    }

    rocksdb::Status s_cf, s_db;
    ...
    if (!cf_options.empty()) {
        s_cf = db->SetOptions(cf_options);
    }

    if (!db_options.empty()) {
        s_db = db->SetDBOptions(db_options);
    }

    fprintf(stderr, "\n[Dynamic Update] Triggered!\n");

    if (!cf_options.empty()) {
        fprintf(stderr, "   -> SetOptions (CF): %s\n",
                s_cf.ok() ? "SUCCESS" : s_cf.ToString().c_str());
    }

    if (!db_options.empty()) {
        fprintf(stderr, "   -> SetDBOptions (DB): %s\n",
                s_db.ok() ? "SUCCESS" : s_db.ToString().c_str());
    }

    if ((cf_options.empty() || s_cf.ok()) &&
        (db_options.empty() || s_db.ok())) {
        fprintf(stderr, "[Dynamic Update] Overall: APPLIED.\n");
    } else {
        fprintf(stderr, "[Dynamic Update] Overall: FAILED.\n");
    }
}
       }
    }
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run() {
    ...
    std::thread listener_thread(DynamicOptionListener, db_.db, &listener_running);
    listener_thread.detach(); // 让它在后台运行，主程序结束时它会被操作系统回收
